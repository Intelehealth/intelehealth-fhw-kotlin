...
interface VisitDao : CoreDao<Visit> {
    ...
    @Query("SELECT sum(completed) as completed, sum(received) as received, sum(pending) as pending, "
                   ...
                   + " encounter_type_uuid = '629a9d0b-48eb-405e-953d-a5964c88dc30') THEN 'Completed' "
                   + " WHEN uuid in (select visituuid from tbl_encounter where encounter_type_uuid ='bd1fbfaa-f5fb-4ebd-b75c-564506fc309e') "
                   + " THEN  'Received' " + "WHEN uuid not in (select visituuid from tbl_encounter where "
                   + " encounter_type_uuid = '629a9d0b-48eb-405e-953d-a5964c88dc30') THEN  'Pending' "
