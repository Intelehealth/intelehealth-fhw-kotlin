...
import org.intelehealth.app.ui.patient.viewholder.PatientViewHolder
 ...
 * @param patients The initial list of prescriptions to display. This list can be updated.
 */
class PatientAdapter(
    context: Context,
    private var patients: LinkedList<ListItemHeaderSection>
) : FooterViewMoreAdapter<ListItemHeaderSection>(context, patients) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return if (viewType != HEADER && viewType != FOOTER) {
            val binding = ListRowItemPatientBinding.inflate(inflater, parent, false)
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (getItemViewType(position) != HEADER && holder is PatientViewHolder && getItem(position) is VisitDetail) {
