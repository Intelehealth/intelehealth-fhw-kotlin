...
import org.intelehealth.app.databinding.ActivityPatientBinding
import org.intelehealth.app.model.ConsentArgs
import org.intelehealth.app.ui.patient.fragment.PatientConsentFragmentArgs
import org.intelehealth.app.ui.patient.fragment.PatientDetailFragmentArgs
 ...
 **/
@AndroidEntryPoint
class PatientActivity : SimpleAppBarActivity() {
    private lateinit var binding: ActivityPatientBinding
    private val afsViewModel: ActiveFeatureStatusViewModel by viewModels()

    private val args: PatientActivityArgs by navArgs<PatientActivityArgs>()

    // Navigation controller for the activity
    private val navController by lazy {
        val navHostFragment =
            supportFragmentManager.findFragmentById(org.intelehealth.app.R.id.navGraphPatient) as NavHostFragment
        navHostFragment.navController
    }

    private val navGraph by lazy {
        navController.navInflater.inflate(org.intelehealth.app.R.navigation.patient_navigation_graph)
    }

    override fun getAppBarBinding(): SimpleAppbarBinding = binding.appBarLayout

    override fun getScreenTitle(): String = getString(ResourceR.string.title_patient_detail)
    ...
    private fun observeActiveFeatureStatus() {
        afsViewModel.fetchActiveFeatureStatus().observe(this) { status ->
            ...
            if (args.patientId == null) setNavigationStartDestination(status)
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        args.patientId?.let {
            navGraph.setStartDestination(R.id.fragmentPatientDetail)
            navController.setGraph(navGraph, PatientDetailFragmentArgs(it).toBundle())
            binding.progressStartDestination.isVisible = false
